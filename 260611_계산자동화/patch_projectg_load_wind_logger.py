# -*- coding: utf-8 -*-
"""
patch_projectg_load_wind_logger.py

ProjectG127.exe의 EntryPoint를 패치해서 같은 폴더의 pangya_wind_logger.dll을
자동 LoadLibraryA 하게 만든다.

이 방식은 CE 값 스캔이 아니라 '클라이언트 EXE 파일 자체 수정' 방식이다.
EXE 시작 직후 DLL이 로드되고, DLL 안에서 클라이언트 함수 hook/로그를 수행한다.

사용:
  python .\patch_projectg_load_wind_logger.py .\ProjectG127.exe

출력:
  ProjectG127.wind_logger_loader.exe

적용 예:
  Copy-Item .\ProjectG127.exe .\ProjectG127.exe.backup_before_wind_logger -Force
  Copy-Item .\ProjectG127.wind_logger_loader.exe .\ProjectG127.exe -Force

필수:
  pangya_wind_logger.dll 을 ProjectG127.exe와 같은 폴더에 둔다.
"""
...
DLL_NAME = b"pangya_wind_logger.dll\x00"
...
class PE:
            ...
            if dll == target_dll:
                ...
                while True:
                    ...
                    if (thunk & 0x80000000) == 0:
                        ...
                        name = bytes(self.data[name_off + 2:self.data.find(b"\x00", name_off + 2)])
                        if name == target_func:
    ...
    def find_code_cave(self, min_size: int = 256) -> tuple[int, int]:
...
def main():
    ...
    cave_off, cave_rva = pe.find_code_cave(512)
    ...
    if len(code) > 0x80:
        raise RuntimeError("loader stub too large")

    patch_blob = bytes(code).ljust(0x80, b"\x90") + DLL_NAME
    data[cave_off:cave_off + len(patch_blob)] = patch_blob
    w32(data, pe.address_of_entry_point_off, cave_rva)

    backup = src.with_name(src.name + ".backup_before_wind_logger_loader")
    if not backup.exists():
        shutil.copy2(src, backup)
        print(f"[INFO] backup created: {backup}")

    out = src.with_name(src.stem + ".wind_logger_loader" + src.suffix)
    out.write_bytes(data)
    print(f"[DONE] output={out}")
    print("")
    print("적용 예:")
    print(f"  Copy-Item .\\{src.name} .\\{src.name}.backup_before_wind_logger -Force")
    print(f"  Copy-Item .\\{out.name} .\\{src.name} -Force")
    print("  # 그리고 pangya_wind_logger.dll / pangya_wind_logger.ini 를 같은 폴더에 둔다")
